  ...
  const fuse = useMemo(
    () => {
      if (!searchData || !Array.isArray(searchData)) {
        console.warn("Search data is missing or not an array");
        return null;
      }
      ...
      >
        <div className="max-w-[1400px] mx-auto px-6 flex items-center justify-center lg:gap-10">
          ...
          <ul className="hidden lg:flex gap-6 items-center">
            ...
            <li className="flex gap-4 ml-2 items-center">
              {/* Search Trigger */}
              <button
                onClick={() => setOpen(true)}
                className={`${iconColorClass} hover:text-green-400 transition-colors cursor-pointer p-1.5 hover:bg-white/10 rounded-lg`}
                title="Search (Cmd+K)"
              >
                <Search size={22} />
              </button>
          ...
          <div className="fixed inset-0 z-[60]">
            ...
            >
              ...
              <div className="w-full max-w-md mb-8 relative">
                ...
                {/* Quick Links / Trending (when empty) */}
                <AnimatePresence>
                  {!searchQuery && (
                    <motion.div
                    ...
                    >
                      <span className="text-[10px] uppercase font-bold text-white/40 w-full text-center mb-1">Quick Links</span>
                      {[
                        ...
                        <Link
                          ...
                          href={link.href}
                          onClick={() => setOpen(false)}
                          className="px-3 py-1 bg-white/5 hover:bg-white/20 border border-white/10 rounded-full text-[11px] font-medium transition-colors"
                        >
                          {link.label}
                        </Link>
                      ))}
